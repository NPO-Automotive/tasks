...
const makeMapStateToProps = () => {
  ...
  const selectNotificationsTotalByCardId =
    selectors.makeSelectNotificationsTotalByCardId();

  return (state, { id, index }) => {
    const { projectId } = selectors.selectPath(state);
    const allProjectsToLists =
      selectors.selectProjectsToListsForCurrentUser(state);
    const allBoardMemberships =
      selectors.selectMembershipsForCurrentBoard(state);
    ...
    const attachmentsTotal = selectors.selectAttachmentsTotalByCardId(
      state,
      id,
    );
  ...
  bindActionCreators(
    {
      ...
      onTransfer: (boardId, listId) =>
        entryActions.transferCard(id, boardId, listId),
