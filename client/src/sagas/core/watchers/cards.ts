...
export default function* cardsWatchers() {
  yield all([
    ...
    takeEvery(EntryActionTypes.CARD_DUPLICATE, ({ payload: { id } }) =>
      services.duplicateCard(id),
    ),
    takeEvery(EntryActionTypes.CURRENT_CARD_DUPLICATE, () =>
      services.duplicateCurrentCard(),
    ),
    ...
    takeEvery(
      EntryActionTypes.TEXT_FILTER_IN_CURRENT_BOARD,
      ({ payload: { text } }) => services.handleTextFilter(text),
    ),
  ]);
}
